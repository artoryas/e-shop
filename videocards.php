<?php
	include 'header.php';
	$videocards=R::getAll('select * from products where type="videocard"');
	$data = $_POST;
	if (isset($data['addToBag'])) {
		...
		if (isset($_SESSION['logged_user'])) {
			if (R::count('bag',"client = ? AND product = ?", array($_SESSION['logged_user']->id,$data['prodId'])) > 0 ) {
				...
				$logtext = "Товар c ID = ".$data['prodId']." добавлен в корзину пользователем ".$_SESSION['logged_user']->login;
				addLog($logtext);
			}
	}else{
	...
	if (isset($data['deleteFromDB'])) {
		R::exec('delete from products where id = "'.$data['prodId'].'"');
			...
			<?php 
				foreach($videocards as $row){
   				...
   				<form method='POST'>
						...
						<div class='amount'><b>Количество: </b><input type='number' class='num' name='amount' value='1' min='1' style='width:100px'></div><br>
